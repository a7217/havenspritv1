...
export default function DepartmentsPage() {
  ...
  const handleToggleActive = async (d: Dept) => {
    try {
      ...
      const json = await res.json();
      if (json.success) setData((prev) => prev.map((x) => x._id === d._id ? json.data : x));
    } catch { /* silent */ }
  };
  ...
  const allSelected = displayed.length > 0 && displayed.every((d) => selected.includes(d._id));
  const someSelected = displayed.some((d) => selected.includes(d._id));

  const toggleAll = () => {
    const ids = displayed.map((d) => d._id);
    if (allSelected) {
  ...
  const handleBulkDelete = async () => {
    ...
    if (!confirm(`${selected.length} departments delete karna chahte ho? Yeh permanently hat jayenge.`)) return;
    setBulkDeleting(true);
    try {
      await Promise.all(
        selected.map((id) => fetch(`/api/departments/${id}`, { method: "DELETE" }))
      );
      setData((prev) => prev.filter((d) => !selected.includes(d._id)));
      setSelected([]);
    } catch { /* silent */ }
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
            <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100 flex-wrap gap-2">
                ...
                {someSelected && (
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      ...
                      className="flex items-center gap-1.5 bg-red-500 hover:bg-red-600 disabled:opacity-60 text-white font-bold px-3 py-1.5 rounded text-[10px] tracking-wider transition-colors"
                    >
              ...
              <div className="flex items-center gap-2">
                ...
                <select value={filterType} onChange={(e) => { setFilterType(e.target.value); setSelected([]); }}
              ...
              <table className="w-full text-xs">
                ...
                <tbody className="divide-y divide-gray-100">
                  {loading ? (
                    ...
                    <tr><td colSpan={5} className="px-4 py-10 text-center text-gray-400">Koi department nahi mili.</td></tr>
                  ) : displayed.map((d) => (
                    <tr key={d._id} className={`transition-colors ${selected.includes(d._id) ? "bg-red-50" : "hover:bg-gray-50"}`}>
